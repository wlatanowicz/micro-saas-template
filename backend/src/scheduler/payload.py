...
class TaskPayload:
    ...
    def from_dict(cls, data: dict[str, Any]) -> TaskPayload:
        ...
        retry_raw = data.get("retry", 0)
        if not isinstance(retry_raw, int):
            msg = f"payload retry must be an int, got {type(retry_raw).__name__}"
            raise TypeError(msg)
        return cls(
    ...
    def with_retry(self, retry: int) -> TaskPayload:
        return TaskPayload(
            function_path=self.function_path,
            args=self.args,
            kwargs=self.kwargs,
